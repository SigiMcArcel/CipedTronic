...
aci_status_code_t do_aci_setup(aci_state_t *aci_stat)
{
  ...
  if(ACI_QUEUE_SIZE-1 >= aci_stat->aci_setup_info.num_setup_msgs)
  {
  ...
  else
  {
    ...
    {
     status = aci_setup(aci_stat, aci_stat->aci_setup_info.num_setup_msgs % (ACI_QUEUE_SIZE-1), ((ACI_QUEUE_SIZE-1)*i));               
    }
  }
